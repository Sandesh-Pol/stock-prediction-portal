...
const APIDialog = ({ isOpen, onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSuccess("Prediction saved successfully! Redirecting…"); // ✅ show success
      setTimeout(() => {
        onClose();
        navigate("/dashboard");
      }, 1500); // short delay so user can see message
    } catch (err) {
      setError(
        err.response?.data?.message || "Failed to fetch data. Try again."
      );
    } finally {
      setLoading(false);
    }
